	...
	<section id="blocPrincipalPage">
		...
		<section id="blocDroitPrincipalPage">
			...
			<div id="blocPhoto">
				...
				<c:if
					test="${rangUtilisateur =='administrateur' || (rangUtilisateur == 'redacteur' && pageGere == urlPage)}">
					<!-- Ajouter la page géré par le rédacteur -->
					<form method="POST">
			...
			<c:if
				test="${rangUtilisateur =='administrateur' || (rangUtilisateur == 'redacteur' && pageGere == urlPage)}">
				<!-- Ajouter la page géré par le rédacteur -->
				<a id="lienNouvelArticle" class="bouttonAjoutArticle"
					href="nouvelarticle?nompage=${urlPage}">Ecrire un nouvel
					article</a>
			</c:if>
			...
			<div id="blocArticle">
				<c:forEach var="listeArticle" items="${listeArticle}">
					<c:if test="${listeArticle.getArchive()==false}">
						<c:if
							test="${listeArticle.getVisiblePage()==true || (rangUtilisateur =='administrateur' || (rangUtilisateur == 'redacteur' && pageGere == urlPage))}">
							<div class='unArticleDeLaPage'>


								<article class='monArticleParticulier'
									id='monArticleParticulier${listeArticle.getIdArticle()}'>
									<div class='cacherConnexion'>
										<c:if
											test="${rangUtilisateur =='administrateur' || (rangUtilisateur == 'redacteur' && pageGere == urlPage)}">
											<!-- Ajouter la page géré par le rédacteur -->
											<a
												href='modifierarticle?nompage=${urlPage}&idArticle=${listeArticle.getIdArticle()}'><button
													class='bouttonArticle bouttonModifierArticle'
													onclick='bouttonOuvrirModificationArticle(this)'
													id='modifierArticle${listeArticle.getIdArticle()}'
													type='button'>Modification</button></a>
											...
											<button class='bouttonArticle bouttonSupprimerArticle'
												...
												type='button'>Suppression</button>
										</c:if>
									</div>
