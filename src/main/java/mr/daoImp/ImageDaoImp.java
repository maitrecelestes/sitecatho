...
public class ImageDaoImp implements ImageDao {
	...
	public List<Image> listeImageCategorie(int idCategorie) {
		...
		try {
			...
			PreparedStatement stmt = connection
					.prepareStatement("SELECT * FROM `photo` WHERE idCategoriePhoto=? ORDER BY `dateCreation` DESC ");
			stmt.setInt(1, idCategorie);
			ResultSet results = stmt.executeQuery();
			while (results.next()) {
				Image image = new Image(results.getInt("id"),
	...
	public void ajouterImage(Image addimage, String ipPosteur) {
		...
		try {
			connection = DataSourceProvider.getDataSource().getConnection();
			PreparedStatement stmt = connection
					.prepareStatement("INSERT INTO `photo`(`lienPhoto`,`idCategoriePhoto`, `mailAuteur`, `dateCreation`, `ipPosteur`) VALUES (?,?,?,NOW(),?)");
	...
	public void supprimerTouteImageCategorie(int idCategorie) {
		...
		try {
			connection = DataSourceProvider.getDataSource().getConnection();
			PreparedStatement stmt = connection
					.prepareStatement("DELETE FROM `photo` WHERE idCategoriePhoto=?");
			stmt.setInt(1, idCategorie);
			stmt.executeUpdate();
			connection.close();
		} catch (SQLException e) {
