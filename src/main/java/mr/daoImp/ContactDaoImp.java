...
public class ContactDaoImp implements ContactDao{
	...
	public List<Contact> listeMessageContact() {
		...
		try {
			...
			while (results.next()) {
				Contact contact= new Contact(results.getInt("idMessage"), results.getString("nom"),results.getString("prenom"),results.getString("mail"),results.getString("objet"),results.getString("contenu"),results.getString("ipPosteur"),results.getDate("datePoste"),results.getTime("heurePoste"));
	...
	public void ajouterContact(Contact newContact) {
		...
		try {
			connection = DataSourceProvider.getDataSource().getConnection();
			PreparedStatement stmt= connection.prepareStatement("INSERT INTO `contact`(`nom`, `prenom`, `mail` ,`objet`, `contenu`, `ipPosteur`, `datePoste`, `heurePoste`) VALUES (?,?,?,?,?,?,NOW(),NOW())");
